const timeObject = {
  ...
  handleConputedDate: (i, times, type, enabledDate, self, enable) => {
    ...
    if (enable) {
      ...
      if (type === 'year') {
        newTimes.undo = Number(newTimes.num) >= startDate && Number(newTimes.num) <= endDate;
        newTimes.undo = !newTimes.undo;
      } else if (type === 'month') {
        if (newYear === startYear && newYear !== endYear) { // 如果等于开始
          newTimes.undo = Number(newTimes.num) < startDate;
        } else if (newYear === endYear && newYear !== startYear) {
          newTimes.undo = Number(newTimes.num) > endDate;
        } else if (newYear === endYear && newYear === startYear) {
          newTimes.undo = Number(newTimes.num) >= startDate && Number(newTimes.num) <= endDate;
          newTimes.undo = !newTimes.undo;
        } else {
